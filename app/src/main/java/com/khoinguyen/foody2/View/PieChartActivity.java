...
//            Log.d("kiemtramaquanan", maquanan);
    ...
    private void DuLieuBieuDo () {
        PieDataSet dataSet = new PieDataSet(newEntries, "Sample Data");
        ...
        pieChart.getDescription().setText("Sample Pie Chart");
        pieChart.setCenterText("Năm " + thoigian);
        pieChart.animate();
    }
    ...
    private void ThongKeSoLuongQuanAn () {
        ...
        // Chuyển đổi Map<Integer, List<QuanAnModel>> sang HashMap<String, Integer>
        HashMap<String, Integer> newMap = new HashMap<>();
        for (Map.Entry<Integer, List<QuanAnModel>> entry : danhSachQuanAnTheoThang.entrySet()) {
            newMap.put("Tháng " +  entry.getKey(), entry.getValue().size());
        }

        newEntries = new ArrayList<>();
        for (Map.Entry<String, Integer> entry : newMap.entrySet()) {
            newEntries.add(new PieEntry(entry.getValue(), entry.getKey()));
        }
